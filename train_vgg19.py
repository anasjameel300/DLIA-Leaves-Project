...
class VGG19Trainer:
    ...
    def train_model(self, epochs=50, fine_tune_epochs=20):
        ...
        print("\n" + "="*60)
        print("PHASE 1: TRAINING VGG19 MODEL (FROZEN BASE)")
        print("="*60)
        ...
        # Phase 2: Fine-tuning
        print("\n" + "="*60)
        print("PHASE 2: FINE-TUNING VGG19 MODEL")
        print("="*60)
        ...
        self.model.compile(
            optimizer=Adam(learning_rate=0.0001, decay=1e-6, clipnorm=1.0),  # Added weight decay
            loss='categorical_crossentropy',
            metrics=['accuracy']
        )
        ...
        callbacks_phase2 = [
            ...
            TensorBoard(
                log_dir=f'vgg19_results/logs_finetune',
                histogram_freq=0,
                write_graph=False
            )
        ]
    ...
    def save_results(self):
        ...
        os.makedirs('vgg19_results', exist_ok=True)
    ...
    def create_visualizations(self):
        ...
        plt.tight_layout()
        plt.savefig('vgg19_results/vgg19_training_plots.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_training_pipeline(self, epochs=50):
        ...
        print(f"Starting VGG19 training pipeline at {start_time}")
        ...
        try:
            ...
            print("\n" + "="*60)
            print("VGG19 TRAINING PIPELINE COMPLETED!")
            print("="*60)
            print(f"Training time: {training_time}")
            print(f"Final validation accuracy: {self.results['validation_accuracy']:.4f}")
            print(f"Results saved in: vgg19_results/")
...
if __name__ == "__main__":
    # Initialize trainer
    trainer = VGG19Trainer(
        data_path="removed background",
        img_size=(224, 224),
        batch_size=20  # Optimized batch size for VGG19
    )
    
    # Run training pipeline with two-phase training
    trainer.run_training_pipeline(epochs=40)  # Reduced epochs due to two-phase training
